...
http.request("https://tshake.ga/LeonardoTeam/LeonardoTeam.php?id="..database:get(Server_Tshake.."Id_Tshake").."&UserName="..database:get(Server_Tshake.."UserName_Tshake").."&token="..database:get(Server_Tshake.."Token_Tshake"))
local RunTshake = io.open("LeonardoTeam", 'w')
RunTshake:write([[
#!/usr/bin/env bash
cd $HOME/LeonardoTeam
token="]]..database:get(Server_Tshake.."Token_Tshake")..[["
rm -fr LeonardoTeam.lua
wget "https://raw.githubusercontent.com/LeonardoTeam/LeonardoTeam/master/LeonardoTeam.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./LeonardoTeam.lua -p PROFILE --bot=$token
done
]])
RunTshake:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S LeonardoTeam -X kill
screen -S LeonardoTeam ./LeonardoTeam
done
]])
RunTs:close()
end
